...
import per.wsj.commonlib.downloader.data.Ranges;
import per.wsj.commonlib.downloader.db.Db;
import per.wsj.commonlib.downloader.net.OkHttpManager;
...
public class FileTask implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (Util.isFileExists(saveFile) && Util.isFileExists(tempFile) && data != null && data.getStatus() != PROGRESS) {
                Response response = OkHttpManager.getInstance().initRequest(url, data.getLastModify());
                if (response != null && response.isSuccessful() && Util.isNotServerFileChanged(response)) {
                ...
                if (response != null && response.isSuccessful()) {
                    if (Util.isSupportRange(response)) {
                        prepareRangeFile(response);
                        saveRangeFile();
                    } else {
    ...
    private void prepareRangeFile(Response response) {
        ...
        try {
            ...
            onStart(fileLength, 0, Util.getLastModify(response), true);

            Db.getInstance(context).deleteData(url);
            Util.deleteFile(saveFile, tempFile);

            saveFile = Util.createFile(path, name);
            tempFile = Util.createFile(path, name + ".temp");
            ...
            Util.close(saveRandomAccessFile);
            Util.close(tempRandomAccessFile);
            Util.close(tempChannel);
            Util.close(response);
        }
    }
    ...
    private void saveRangeFile() {
        ...
        final File saveFile = Util.createFile(path, name);
        final File tempFile = Util.createFile(path, name + ".temp");
    ...
    private void saveCommonFile(Response response) {
        ...
        try {
            ...
            Util.deleteFile(path, name);

            File file = Util.createFile(path, name);
            if (file == null) {
                return;
            }
